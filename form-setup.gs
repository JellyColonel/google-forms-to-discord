    ...
    if (title.includes(CONFIG.form.discordIdIdentifier)) {
      ...
      textItem.setValidation(
        FormApp.createTextValidation()
          ...
          .setHelpText('Discord ID должен состоять исключительно из цифр, и быть длинной от 17 до 19 символов')
          .build()
      );
    }
    ...
    if (title.includes(CONFIG.form.multipleChoiceQuestionName)) {
      ...
      multipleChoiceItem.setChoices(
        validDepartments.map(dept => 
          multipleChoiceItem.createChoice(dept)
        )
      );
    }
  });
...
function resetFormValidation() {
  ...
  items.forEach(item => {
    try {
      ...
      console.error(`Error resetting validation for item ${item.getTitle()}:`, error);
    }
  });
}
